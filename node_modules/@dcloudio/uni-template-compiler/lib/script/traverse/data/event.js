...
function parseEvent (keyPath, valuePath, state, isComponent, isNativeOn = false) {
...
function _processEvent (path, state, isComponent, isNativeOn = false) {
  const opts = []
...
module.exports = function processEvent (paths, path, state, isComponent) {
  const onPath = paths['on']
  const nativeOnPath = paths['nativeOn']

  const opts = []
  if (onPath) {
    _processEvent(onPath, state, isComponent).forEach(opt => {
      opts.push(opt)
    })
  }
  if (nativeOnPath) {
    _processEvent(nativeOnPath, state, isComponent, true).forEach(opt => {
      opts.push(opt)
    })
  }
  if (!opts.length) {
